...
@section('content')
    <h1 class="text-2xl font-semibold text-slate-900 mb-4">Edit testimonial</h1>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" class="bg-white/80 rounded-xl border border-slate-200 shadow-sm p-6 max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="number" name="rating" value="{{ old('rating', $testimonial->rating) }}" min="1" max="5"
                       class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#a35151]/50">
            </div>
            ...
            <div>
                ...
                <textarea name="message" rows="4" required
                          class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#a35151]/50">{{ old('message', $testimonial->message) }}</textarea>
            </div>
        </div>
